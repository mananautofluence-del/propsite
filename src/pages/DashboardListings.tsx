...
import { Search, Grid3X3, List, Copy, MessageCircle, MoreHorizontal, Eye, Clock, Pencil } from 'lucide-react';
  ...
  return (
    <DashboardLayout title="My Listings">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex border border-border rounded-md overflow-hidden">
              <button onClick={() => setView('grid')} className={`w-8 h-8 flex items-center justify-center ${view === 'grid' ? 'bg-surface-2' : 'bg-surface'}`}>
                <Grid3X3 size={14} className="text-text-2" />
              </button>
              <button onClick={() => setView('list')} className={`w-8 h-8 flex items-center justify-center ${view === 'list' ? 'bg-surface-2' : 'bg-surface'}`}>
        ...
        {loading ? (
          <div className="text-center py-16 text-sm text-text-3">Loading listings...</div>
        ) : view === 'grid' ? (
              ...
              <div key={l.id} className="card-base p-0 overflow-hidden group">
                ...
                <div className="p-3">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-2xs text-text-3">
                    <span className="flex items-center gap-1"><Eye size={10} /> {l.total_views || 0}</span>
                  </div>
                  <div className="border-t border-border mt-2.5 pt-2.5 flex items-center gap-1.5">
                    <button onClick={() => copyLink(l.slug)} className="btn-primary text-2xs h-7 px-3 flex items-center gap-1"><Copy size={10} /> Copy Link</button>
                    <button className="w-7 h-7 rounded-md border border-border flex items-center justify-center hover:bg-surface-2">
                  ...
                  <tr key={l.id} className="border-b border-border last:border-0 h-11 hover:bg-surface-2/50">
                    <td className="px-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-10 h-7 rounded bg-surface-2 overflow-hidden">
                          {getHeroUrl(l) ? <img src={getHeroUrl(l)} alt="" className="w-full h-full object-cover" /> : null}
                        </div>
                    ...
                    <td className="px-3">
                      <button onClick={() => copyLink(l.slug)} className="btn-primary text-2xs h-6 px-2"><Copy size={10} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-16">
            ...
            <Link to="/dashboard/listings/new" className="btn-primary inline-flex items-center">Create Listing</Link>
          </div>
        )}
      </div>
    </DashboardLayout>
